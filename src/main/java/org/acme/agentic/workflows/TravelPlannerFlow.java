...
public class TravelPlannerFlow {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(TravelPlannerFlow.class);
    @Inject
    ...
    public void onBudgetFlightPoolerResult(@Observes CloudEvent e) {
        LOGGER.info("Received budget plan event for flightPoolerResult {}", e);
    }
    ...
    public Workflow flightPriceWatcherFlow() {
        return workflow("flight-price-watcher")
                ...
                .next(operation().action(
                        call(log("logLowerBudget", WorkflowLogLevel.INFO), jsonObject().put(SysOutTypeHandler.SYSOUT_TYPE_PARAM, "Airfare Value is bellow the budget, we are ok")))).end()
                .when(".airfareValue > .airfareBudget")
                .next(operation().action(
                        call(log("logHigherBudget", WorkflowLogLevel.INFO), jsonObject().put(SysOutTypeHandler.SYSOUT_TYPE_PARAM, "Airfare Value is higher then the budget, let's start pooling for airfare prices"))))
                ...
                .next(operation()
                        ...
                        .action(
                                call(FunctionBuilder.java("notifyPooling", notificationService::notifyPooling),
                                        ".userAddress", "Your travel plan is on holding, we will check with Flight companies a better price under your budget.")))
                .next(callback(
                        call(FunctionBuilder.java("findCheaperFlightAsync", budgetFlightPooler::findCheaperFlightAsync),
                                ".flightRequest", "{ budget: .airfareBudget, attempts: 10, intervalMs: 1000 }", "$WORKFLOW.id"), eventDef(Events.FLIGHT_POOLER_RESULT))).end()
                .end().build();
    }
